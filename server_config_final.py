...
GRID_TICK_SECONDS = 3  # Full bot cycle interval (grid orders, indicators, etc.) — reduced from 5s for faster order replacement
...
INDICATOR_CACHE_TTL_1M = 30  # Increased from 15 to reduce API rate limits
INDICATOR_CACHE_TTL_5M15M = 120  # Increased from 60 to reduce API rate limits
...
SMALL_CAPITAL_DEFAULT_PROFILE = "MEME"
...
MAX_BOT_LOSS_PCT = 0.05  # 5% max loss per bot
...
ENABLE_BTC_CORRELATION_FILTER = True  # Enable/disable correlation filter
...
SL_MAX_DISTANCE_PCT = 0.15  # Maximum 15% SL distance from current price
...
ENABLE_TREND_PROTECTION = True  # Set False to disable trend protection
...
# Reduced from 0.70 to 0.50 for more responsive trailing in directional modes
RANGE_RECENTER_THRESHOLD_PCT = (
    0.50  # Recenter when price reaches 50% to either boundary
)
...
ENABLE_DANGER_ZONE_DETECTION = True  # Set False to disable
...
GLOBAL_KILL_SWITCH_ENABLED = (
    True  # Set True to enable global stop on MAX_DAILY_LOSS_PCT
)

# Volatility freeze thresholds - pause recentering when volatility is extreme
VOLATILITY_FREEZE_ATR_PCT = 0.03  # 3% ATR threshold to freeze recentering
VOLATILITY_FREEZE_BBW_PCT = 0.08  # 8% BBW threshold to freeze recentering
...
AUTO_MARGIN_PRESETS = {
    # Conservative: small top-ups to push liq distance into the ~15–20% range,
    # with hard caps to avoid draining available balance in one go.
    "conservative": {
        "enabled": True,
        "min_trigger_pct": 18.0,
        "target_liq_pct": 25.0,
        "cooldown_sec": 12,
        "max_add_ratio": 0.75,  # up to 75% of available USDT
        "min_add_usdt": 0.1,  # allow small adds so low balances still work
        "max_add_usdt": 8.0,  # per-fire cap
        "max_total_add_usdt": 25.0,  # total cap per bot (0 = no cap)
        "position_idx": 0,
        "critical_pct": 7.0,
    },
    # Aggressive: previous behavior (larger, uncapped adds).
    "aggressive": {
        "enabled": True,
        "min_trigger_pct": 20.0,
        "target_liq_pct": 30.0,
        "cooldown_sec": 8,
        "max_add_ratio": 0.9,
        "min_add_usdt": 0.1,
        "max_add_usdt": 150.0,
        "max_total_add_usdt": 0.0,  # uncapped
        "position_idx": 0,
        "critical_pct": 8.0,
    },
}
...
MARGIN_MONITOR_TRIGGER_PCT = 18.0  # Add margin when pct_to_liq <= 18%
MARGIN_MONITOR_TARGET_PCT = 30.0  # Target liq distance after adding margin
MARGIN_MONITOR_CRITICAL_PCT = 10.0  # Skip cooldown when below 10%
MARGIN_MONITOR_COOLDOWN_SEC = 8  # Seconds between adds per position
MARGIN_MONITOR_MAX_ADD_RATIO = 0.80  # Max 80% of available balance per add
MARGIN_MONITOR_MIN_ADD_USDT = 0.1  # Minimum add amount
MARGIN_MONITOR_MAX_ADD_USDT = 150.0  # Maximum add amount per action
MARGIN_MONITOR_ALL_POSITIONS = True  # Monitor ALL positions, not just bot positions
MARGIN_MONITOR_EMERGENCY_MAX_PCT_PER_BOT = (
    0.40  # Max 40% of available balance per bot (emergency)
)
MARGIN_MONITOR_EMERGENCY_MAX_PCT_TOTAL = (
    0.80  # Max 80% of available balance per hour (emergency)
)
...
OUT_OF_RANGE_WAIT_SEC = 60  # Wait 60s before acting (avoid false breakouts)
...
SR_MIN_TOUCHES = 2  # Minimum touches to form valid S/R level
SR_LOOKBACK_CANDLES = 500  # Candles for S/R detection (was 100)
...
PREDICTION_NORM_STRONG_THRESHOLD = 70  # score_norm >= 70 for STRONG_LONG/SHORT
...
ENABLE_FLASH_CRASH_PROTECTION = True  # Master toggle for flash crash protection
...
ENABLE_FUNDING_RATE_SIGNAL = True  # Master toggle
...
FUNDING_PROTECTION_ENABLED = True  # Skip new orders near funding time
...
BTC_GUARD_ENABLED = True  # Master toggle
...
SMART_ENTRY_ENABLED = True  # Master toggle
...
# Prevents entering at overbought/oversold extremes or extended prices

ENTRY_GATE_ENABLED = True  # Master toggle

# Long mode thresholds (block if exceeded - avoid buying at tops)
ENTRY_GATE_RSI_LONG_MAX = 80  # Block if RSI > 80 (extreme overbought)
ENTRY_GATE_BB_LONG_MAX = 0.95  # Block if BB position > 95% (near upper band)
ENTRY_GATE_EMA_LONG_MAX = 0.05  # Block if price > 5.0% above EMA21

# Short mode thresholds (block if below - avoid selling at bottoms)
ENTRY_GATE_RSI_SHORT_MIN = 35  # Block if RSI < 35 (oversold)
ENTRY_GATE_BB_SHORT_MIN = 0.15  # Block if BB position < 15% (near lower band)
ENTRY_GATE_EMA_SHORT_MAX = 0.015  # Block if price > 1.5% below EMA21

# Gate behavior
ENTRY_GATE_RECHECK_SECONDS = 60  # Recheck interval when blocked
ENTRY_GATE_TIMEFRAME = "15"  # Timeframe for indicator checks
...
PARTIAL_TP_LEVELS = [  # List of (profit_pct, close_pct) tuples
    (0.003, 0.40),  # At 0.5% profit, close 30%
    (0.006, 0.35),  # At 1.0% profit, close 40% more
    (0.010, 0.25),  # At 2.0% profit, close remaining 30%
]
...
ENABLE_TREND_REVERSAL_PROTECTION = True  # Master toggle
...
EMERGENCY_PARTIAL_CLOSE_ENABLED = True  # Master toggle
...
EMERGENCY_PARTIAL_CLOSE_PCT = 20.0  # Tier 1: Close 20% of position
EMERGENCY_PARTIAL_CLOSE_TIER2_PCT = 35.0  # Tier 2: Close 35% of position
EMERGENCY_PARTIAL_CLOSE_COOLDOWN = 60  # Seconds between partial closes
EMERGENCY_PARTIAL_CLOSE_MAX_COUNT = 10  # Death spiral protection: max closes before full close + stop
...
MOMENTUM_DCA_ENABLED = True  # Master toggle
...
UPNL_STOPLOSS_SYMBOL_DEFAULTS: Dict[str, Dict[str, Any]] = {
    "ETHUSDT": {
        "max_position_pct": 70,  # Position cap for ETH (less volatile major)
    ...
    "__default__": {
        "max_position_pct": 50,  # Position cap for alts/memes (more volatile)
...
NEUTRAL_BREAKOUT_THRESHOLD_PCT = 0.002  # 0.2% above upper / below lower grid
...
NEUTRAL_INVENTORY_CAP_PCT = 0.25  # 25% of target notional (investment*leverage)
...
NEUTRAL_MAX_LOSS_USD = 1.0  # Max unrealized loss in USD (safe for small)
...
NEUTRAL_MOMENTUM_ADX_THRESHOLD = 35  # Block if 1m ADX > 35
NEUTRAL_MOMENTUM_RSI_UPPER = 65  # Block if 1m RSI > 65 (overbought)
NEUTRAL_MOMENTUM_RSI_LOWER = 35  # Block if 1m RSI < 35 (oversold)
...
NEUTRAL_PRESETS = {
    "MAJOR": {
        ...
        "momentum_adx_threshold": 35,  # ADX < 35 allowed (relaxed from 28)
    ...
    "MEME": {
        ...
        "momentum_adx_threshold": 35,  # ADX < 35 (relaxed from 22)
...
NEUTRAL_GATE_ADX_15M_MAX = 35  # Block if ADX > 35 (trending)
NEUTRAL_GATE_RSI_15M_UPPER = 65  # Block if RSI > 65 (overbought)
NEUTRAL_GATE_RSI_15M_LOWER = 35  # Block if RSI < 35 (oversold)
...
NEUTRAL_GATE_ADX_1M_MAX = 40  # Block if 1m ADX > 40 (strong move)
...
# =============================================================================
# Smart Rotation (added to match server grid_bot_service.py imports)
# =============================================================================
...
# =============================================================================
EMERGENCY_PARTIAL_CLOSE_ENABLED = True
EMERGENCY_PARTIAL_CLOSE_LIQ_PCT = 5.0
EMERGENCY_PARTIAL_CLOSE_PCT = 0.25
EMERGENCY_PARTIAL_CLOSE_TIER2_LIQ_PCT = 3.0
EMERGENCY_PARTIAL_CLOSE_TIER2_PCT = 0.50
EMERGENCY_PARTIAL_CLOSE_COOLDOWN = 300
EMERGENCY_PARTIAL_CLOSE_MAX_COUNT = 3
