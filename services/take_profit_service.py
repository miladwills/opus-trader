...
class TakeProfitService:
    ...
    def calculate_take_profit(
        ...
        if volatility_pct is None or volatility_pct <= 0:
            ...
            if mode in ("scalp_pnl", "scalp_market"):
                ...
                tp_price_distance_pct = 0.008  # 0.8% for neutral
                tp_method = "default_neutral_no_volatility"
                reasons.append("No volatility data, using neutral default 0.8%")
            else:
                tp_price_distance_pct = 0.010  # 1.0% for directional
                tp_method = "default_directional_no_volatility"
                reasons.append("No volatility data, using directional default 1.0%")
        else:
            ...
            if mode in ("scalp_pnl", "scalp_market"):
                ...
                tp_price_distance_pct = min(base_tp_distance, grid_width_pct * 0.15)  # 15% of grid width max
                tp_method = "volatility_neutral_gridbound"
                reasons.append(f"Neutral mode: min({base_tp_distance*100:.3f}%, grid_width*0.15={grid_width_pct*0.15*100:.3f}%)")

            elif mode in ("long", "short"):
                # Directional: More aggressive TP (aiming for trend profits)
                tp_price_distance_pct = base_tp_distance * 0.8  # Less aggressive for small capital
                tp_method = "volatility_directional"
                reasons.append(f"Directional mode: {base_tp_distance*100:.3f}% × 0.8 = {tp_price_distance_pct*100:.3f}%")
    ...
    def should_take_profit(
        ...
        """
        if require_positive_pnl and unrealized_pnl <= 0:
            return {
                "should_close": False,
                "reason": (
                    f"PnL not positive enough for TP: ${unrealized_pnl:.2f}"
                ),
                "pnl": unrealized_pnl,
