...
def _acquire_lock_platform(fd, exclusive=True, non_blocking=False):
    ...
    if sys.platform == "win32":
        ...
        # LK_NBLCK: Non-blocking
        # LK_LOCK: Blocking (wait 10s)
        mode = msvcrt.LK_NBLCK if non_blocking else msvcrt.LK_LOCK
        try:
            # Lock the first byte
            # On Windows, we need to ensure the file has at least 1 byte to lock
...
def file_lock(lock_path, exclusive=False):
    ...
    # If file doesn't exist (touched above), r+ works.
    f = open(path_obj, "r+")
    try:
        # On Windows, we ensure file has content to lock
...
def acquire_process_lock(lock_path):
    """
    Acquire a persistent exclusive lock (non-blocking).
    Returns file object if successful, None if lock held.
    """
    path_obj = Path(lock_path)
    path_obj.parent.mkdir(parents=True, exist_ok=True)

    # We use a+ to append/create
    f = open(path_obj, "a+")
    try:
        # On Windows, we ensure file has content to lock
        if sys.platform == "win32" and os.path.getsize(path_obj) == 0:
