...
// =============================================================================
// AI Advisor / Guardian Badge
// =============================================================================
function aiGuardBadge(bot) {
  if (!bot.ai_advisor_enabled) return "";

  const decision = bot.last_ai_decision || {};
  const lastAction = decision.action || null;
  const regime = decision.regime || "?";
  const confidence = decision.confidence != null ? (decision.confidence * 100).toFixed(0) : "?";
  const error = bot.last_ai_error;

  let state = "NO_CHANGE";
  if (bot.ai_trade_allowed_effective === false || lastAction === "PAUSE") {
    state = "PAUSE";
  } else if (lastAction === "ALLOW") {
    state = "ALLOW";
  }

  const stateCls = state === "PAUSE"
    ? "bg-red-600/30 text-red-300"
    : state === "ALLOW"
      ? "bg-emerald-600/30 text-emerald-300"
      : "bg-slate-700/50 text-slate-400";

  const regimeCls = regime === "UP" ? "text-emerald-400"
    : regime === "DOWN" ? "text-red-400"
      : "text-amber-400";

  const tooltip = error ? `Error: ${error}` : `Regime: ${regime}, Confidence: ${confidence}%, Action: ${lastAction || 'N/A'}`;

  return `
    <div class="mt-0.5" title="${tooltip}">
      <div class="flex items-center gap-1 flex-wrap">
        <span class="px-1.5 py-0.5 rounded text-[10px] font-bold ${stateCls}">AI: ${state}</span>
        <span class="px-1 py-0.5 rounded text-[10px] bg-slate-800 ${regimeCls}">${regime}</span>
        <span class="px-1 py-0.5 rounded text-[10px] bg-slate-800 text-slate-300">${confidence}%</span>
      </div>
      ${error ? `<div class="text-[9px] text-amber-400 truncate max-w-[120px]" title="${error}">⚠ ${error.substring(0, 20)}...</div>` : ''}
    </div>
  `;
}
...
// Scanner AI Advisor
// =============================================================================
const scannerAiPreviewCache = new Map();

function scannerAiCellId(symbol) {
  const safe = encodeURIComponent(symbol).replace(/%/g, "_");
  return `scanner-ai-${safe}`;
}

async function fetchScannerAiPreview(symbol) {
  const key = (symbol || "").toUpperCase();
  if (!key) return { success: false, error: "missing_symbol" };

  if (scannerAiPreviewCache.has(key)) {
    return scannerAiPreviewCache.get(key);
  }

  const promise = fetchJSON(`/ai-preview/${encodeURIComponent(key)}`)
    .catch(error => ({ success: false, error: error.message }));
  scannerAiPreviewCache.set(key, promise);
  return promise;
}

function renderScannerAiBadge(preview) {
  if (!preview || preview.success === false) {
    const reason = preview?.error ? `AI error: ${preview.error}` : "AI unavailable";
    return `<span class="text-[10px] text-slate-500" title="${escapeHtml(reason)}">AI N/A</span>`;
  }

  const raw = preview.raw_decision || {};
  const validated = preview.decision || {};
  const action = validated.action || raw.action || "NO_CHANGE";
  const regime = raw.regime || validated.regime || "?";
  const confidence = raw.confidence != null ? Math.round(raw.confidence * 100) : null;
  const mode = raw.recommended_mode || validated.recommended_mode || "-";
  const reasons = raw.reasons || validated.reasons || [];

  const actionCls = action === "ALLOW" ? "text-emerald-400" : action === "PAUSE" ? "text-red-400" : "text-slate-400";
  const regimeCls = regime === "UP" ? "text-emerald-400" : regime === "DOWN" ? "text-red-400" : "text-amber-400";
  const confText = confidence != null ? `${confidence}%` : "?";
  const title = reasons.length ? escapeHtml(reasons.join(" | ")) : "AI advisor";

  return `
    <div class="flex flex-col items-center text-[10px]" title="${title}">
      <span class="font-bold ${actionCls}">${action} ${confText}</span>
      <span class="${regimeCls}">${regime}</span>
      <span class="text-slate-400">${mode}</span>
    </div>
  `;
}

async function updateScannerAiCell(symbol) {
  const cell = document.getElementById(scannerAiCellId(symbol));
  if (!cell) return;
  cell.innerHTML = `<span class="text-[10px] text-slate-500">AI...</span>`;
  const preview = await fetchScannerAiPreview(symbol);
  cell.innerHTML = renderScannerAiBadge(preview);
}
...
async function populateScannerAi(results) {
  if (!Array.isArray(results) || results.length === 0) return;
  const tasks = results.map(r => () => updateScannerAiCell(r.symbol));
...
function recommendedModeBadge(mode, rangeMode, confidence, reasoning) {
  const modeColors = {
  ...
  const confDot = confidence >= 0.75 ? "🟢" : confidence >= 0.6 ? "🟡" : "⚪";
  const cls = modeColors[mode] || modeColors.neutral;
  const icon = modeIcons[mode] || "";
  const rmShort = rangeModeShort[rangeMode] || "F";
  const confPct = Math.round(confidence * 100);
  const tooltip = reasoning ? `title="${reasoning} (${confPct}% confidence)"` : "";
  return `<span class="px-2 py-1 rounded text-xs font-medium ${cls} cursor-help" ${tooltip}>${icon} ${mode}/${rmShort}</span>`;
}
...
async function refreshSummary() {
  ...
  realizedEl.className = `text-sm font-medium ${realizedPnL.class}`;
      ...
      <tr class="table-row">
        ...
        <td class="px-3 py-3 text-center">
          <div class="flex items-center justify-center gap-2">
            ...
            <button id="btn-backtest-${bot.symbol}" onclick="runBacktest('${bot.symbol}')" title="Run Backtest" class="px-2 py-1.5 bg-indigo-600 hover:bg-indigo-500 rounded text-white transition text-xs">🔬</button>
            <button onclick='editBot(${JSON.stringify(bot)})' title="Edit" class="px-2 py-1.5 bg-slate-700 hover:bg-slate-600 rounded text-slate-300 hover:text-white transition text-xs">✏️</button>
            ...
            <button onclick="toggleAiGuard('${bot.id}', '${bot.symbol}', ${bot.ai_advisor_apply === false ? "true" : "false"})" title="Toggle AI guard" class="px-2 py-1.5 ${bot.ai_advisor_apply === false ? 'bg-slate-700 text-slate-200 hover:bg-slate-600' : 'bg-cyan-700 text-white hover:bg-cyan-600'} rounded transition text-xs font-bold">${bot.ai_advisor_apply === false ? 'AI OFF' : 'AI ON'}</button>
...
function updateRunningBotsStatus(bots) {
  ...
  container.innerHTML = runningBots.map(bot => {
    ...
    return `<div class="flex flex-col gap-0.5 p-2 rounded bg-slate-800 border border-slate-700 min-w-[120px]">
      ...
      ${positionInfo ? `<div class="text-[9px] text-amber-400">${positionInfo}</div>` : ''}
...
async function scanNeutral() {
  ...
  tbody.innerHTML = `<tr><td colspan="15" class="px-4 py-8 text-center text-slate-500">Scanning...</td></tr>`;

  try {
    const data = await fetchJSON(url);
    const results = data.results || [];
    if (results.length === 0) {
      tbody.innerHTML = `<tr><td colspan="15" class="px-4 py-8 text-center text-slate-500">No results found</td></tr>`;
      return;
    }
    tbody.innerHTML = results.map(r => {
      ...
      const confidence = r.mode_confidence || 0.5;
      const reasoning = r.mode_reasoning || "";
      const riskData = calculateRisk(r);
      const aiCell = scannerAiCellId(r.symbol || "");
      return `
        <tr class="table-row">
          ...
          <td class="px-1 py-2 text-center w-16" id="${aiCell}">
            <span class="text-[10px] text-slate-600">AI...</span>
          </td>
          ...
          <td class="px-1 py-2 text-center w-20 break-words">${recommendedModeBadge(recMode, recRangeMode, confidence, reasoning)}</td>
    ...
    populateScannerAi(results);
  } catch (error) {
    tbody.innerHTML = `<tr><td colspan="15" class="px-4 py-8 text-center text-red-400">Scan failed: ${error.message}</td></tr>`;
  }
}
...
function fillBotForm(bot) {
  ...
  // Trailing SL checkbox (Smart Feature #14) - default to enabled
...
function resetBotForm() {
  ...
  const volGateEnabled = $("bot-volatility-gate-enabled");
  if (volGateEnabled) volGateEnabled.checked = false;
  const volGateThresh = $("bot-volatility-gate-threshold");
...
async function saveBot() {
  ...
  if (!botData.lower_price || !botData.upper_price || botData.lower_price >= botData.upper_price) { alert("Invalid price range"); return; }
