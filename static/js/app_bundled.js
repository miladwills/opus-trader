console.log("Bybit Control Center v20260112-a Loaded");
const API_BASE = "/api";
...
async function refreshPositions() {
  ...
  // Preserve input values and focus state before regenerating HTML
  const activeElement = document.activeElement;
  const activeId = activeElement?.id || '';
  const isTargetInput = activeId.startsWith('target-input-');
  const preservedValues = {};
  const preservedFocusSymbol = isTargetInput ? activeId.replace('target-input-', '') : null;

  // Save all target input values
  positions.forEach(pos => {
    const input = document.getElementById(`target-input-${pos.symbol}`);
    if (input && input.value) {
      preservedValues[pos.symbol] = input.value;
    }
  });
  ...
  // Restore preserved input values and focus after regenerating HTML
  Object.entries(preservedValues).forEach(([symbol, value]) => {
    const input = document.getElementById(`target-input-${symbol}`);
    if (input) {
      input.value = value;
    }
  });

  // Restore focus if user was typing in a target input
  if (preservedFocusSymbol) {
    const input = document.getElementById(`target-input-${preservedFocusSymbol}`);
    if (input) {
      input.focus();
...
async function refreshBots() {
  ...
  if (bots.length === 0) {
    tbody.innerHTML = `<tr><td colspan="15" class="px-4 py-8 text-center text-slate-500">No bots configured</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = bots.map(bot => {
    ...
    return `
      <tr class="table-row" id="bot-row-${bot.id}">
        ...
        <td class="px-2 sm:px-3 py-3 font-medium">
          <div class="flex items-center gap-1">
            ...
            <button onclick="scanSymbol('${bot.symbol}')" class="text-slate-500 hover:text-emerald-400 transition text-[10px]" title="Search in Scanner">🔍</button>
          </div>
        </td>
        <td class="px-1 sm:px-3 py-3 text-[10px] sm:text-xs" style="display: table-cell !important;">${modeBadge(bot.mode, scalpAnalysis, bot)}</td>
        <td class="px-1 sm:px-3 py-3 text-center text-[10px] sm:text-xs" style="display: table-cell !important;">${rangeModeBadge(rangeMode, widthPct)}</td>
        <td class="px-1 sm:px-3 py-3 text-right text-[10px] sm:text-xs" style="display: table-cell !important;">${bot.grid_count || "-"}</td>
        <td class="px-1 sm:px-3 py-3 text-right text-[10px] sm:text-xs" style="display: table-cell !important;">${bot.leverage || 3}x</td>
        <td class="px-1 sm:px-3 py-3 text-right text-[10px] sm:text-xs" style="display: table-cell !important;">${formatNumber(bot.investment, 0)}</td>
        ...
        <td class="px-2 sm:px-3 py-3 text-right ${pnl.class}" data-bot-pnl="${bot.id}">
          <div class="flex flex-col items-end">
            <span class="font-medium text-[10px] sm:text-sm">${pnl.text}</span>
            <span class="text-[9px] text-slate-500 hidden sm:inline">R: ${formatPnL(bot.realized_pnl, 2).text}</span>
          </div>
        </td>
        ...
        <td class="px-1 sm:px-3 py-3 text-center">
          <div class="flex items-center justify-center gap-1 sm:gap-2">
            ${bot.status !== "running" && bot.status !== "recovering" ? `<button onclick="botAction('start', '${bot.id}', event)" title="Start" class="p-1 sm:px-2 sm:py-1.5 bg-emerald-600 hover:bg-emerald-500 rounded text-white transition text-[10px] sm:text-xs">▶</button>` : ""}
            ${bot.status === "running" ? `<button onclick="botAction('pause', '${bot.id}', event)" title="Pause" class="p-1 sm:px-2 sm:py-1.5 bg-amber-500 hover:bg-amber-400 rounded text-black transition text-[10px] sm:text-xs font-bold">⏸</button>` : ""}
            <button onclick="editBotById('${bot.id}')" title="Edit" class="p-1 sm:px-2 sm:py-1.5 bg-slate-700 hover:bg-slate-600 rounded text-slate-300 hover:text-white transition text-[10px] sm:text-xs">✏️</button>
            <button onclick='showQuickEdit("${bot.id}", ${JSON.stringify({ tp_pct: bot.tp_pct, auto_stop: bot.auto_stop, auto_stop_target_usdt: bot.auto_stop_target_usdt })})' title="Quick Settings" class="p-1 sm:px-2 sm:py-1.5 bg-purple-700 hover:bg-purple-600 rounded text-purple-200 hover:text-white transition text-[10px] sm:text-xs">⚙️</button>
            ${bot.status !== "stopped" ? `<button onclick="botAction('stop', '${bot.id}', event)" title="Stop" class="p-1 sm:px-2 sm:py-1.5 bg-red-600 hover:bg-red-500 rounded text-white transition text-[10px] sm:text-xs font-bold">⏹</button>` : ""}
          </div>
        </td>
      </tr>
    `;
...
function scanSymbol(symbol) {
  ...
  const scannerSection = $('neutral-scanner');
  if (scannerSection) {
...
function editBotById(botId) {
  ...
  if (bot) {
    ...
    // Highlight the bot row in the table by bot ID (not symbol, since symbols can be duplicated)
    const row = document.getElementById(`bot-row-${botId}`);
    if (row) {
      row.scrollIntoView({ behavior: "smooth", block: "center" });
      row.classList.add("bg-purple-500/30", "ring-1", "ring-purple-400");
      setTimeout(() => {
        row.classList.remove("bg-purple-500/30", "ring-1", "ring-purple-400");
      }, 3000);
    }

    // Also scroll to the form after a brief delay
    setTimeout(() => {
...
async function saveBot() {
  ...
  try {
    ...
    const savedBotId = botData.id; // For edits, we have the ID
    ...
    scrollToBotRow(resp?.bot?.id || savedBotId, savedSymbol);
  } catch (error) {
...
function scrollToBotRow(targetBotId, savedSymbol) {
  ...
  const tryScroll = (attempt) => {
    ...
    if (!botId && savedSymbol) {
      // Only fallback when no ID is known; pick newest by symbol
    ...
    if (row) {
      row.scrollIntoView({ behavior: "smooth", block: "center" });
      row.classList.add("ring-2", "ring-purple-400/70", "ring-offset-2", "ring-offset-slate-900");
      ...
      setTimeout(() => {
        ...
        row.style.boxShadow = "";
        row.classList.remove("ring-2", "ring-purple-400/70", "ring-offset-2", "ring-offset-slate-900");
      }, 4000);
  ...
  // Delay slightly to ensure table render completed, then retry if needed
  setTimeout(() => tryScroll(1), 400);
}
...
async function botAction(action, botId, event, silent = false) {
  ...
  if (event && event.target) {
    actionBtn = event.target.closest('button');
    if (actionBtn) {
      actionBtn.classList.add('btn-click-animate');
      setTimeout(() => actionBtn.classList.remove('btn-click-animate'), 200);

      // Visual feedback for start action
      if (action === "start") {
        originalBtnContent = actionBtn.innerHTML;
        actionBtn.innerHTML = "Starting...";
  ...
  if (action === "delete" && !silent && !confirm("Are you sure you want to delete this bot?")) {
    if (actionBtn && originalBtnContent) {
...
function showQuickEdit(botId, settings) {
  ...
  // Look up bot from cache if settings not provided
  if (!settings) {
    const bot = window._lastBots?.find(b => b.id === botId);
    settings = bot ? { tp_pct: bot.tp_pct, auto_stop: bot.auto_stop, auto_stop_target_usdt: bot.auto_stop_target_usdt } : {};
  }
