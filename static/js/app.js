...
const API_BASE = "/api";
const LAST_STARTED_BOT_KEY = 'lastStartedBot';
...
const LIVE_CONNECTED_PNL_REFRESH_MS = 15000;
const LIVE_CONNECTED_FULL_REFRESH_MS = 30000;
const LIVE_FALLBACK_PNL_REFRESH_MS = 3000;
...
// =============================================================================
// Last Coin Quick-Run Feature
// =============================================================================

// Save the last started bot to localStorage (with full settings)
function saveLastStartedBot(bot) {
  console.log('[LastCoin] saveLastStartedBot called with:', bot);
  if (!bot || !bot.id || !bot.symbol) {
    console.log('[LastCoin] Invalid bot data, skipping save');
    return;
  }
  // Save ALL bot config for quick re-run (preserves all settings)
  const lastBotData = {
    id: bot.id,
    symbol: bot.symbol,
    mode: bot.mode || 'long',
    lower_price: bot.lower_price || 0,
    upper_price: bot.upper_price || 0,
    grid_count: bot.grid_count || 10,
    leverage: bot.leverage || 3,
    investment: bot.investment || 50,
    auto_stop_target_usdt: bot.auto_stop_target_usdt || 0,
    range_mode: bot.range_mode || 'trailing',
    profile: bot.profile || 'normal',
    // Advanced settings (using correct field names)
    auto_direction: bot.auto_direction || false,
    auto_pilot: bot.auto_pilot || false,
    trailing_sl_enabled: !!bot.trailing_sl_enabled,
    trailing_sl_activation_pct: bot.trailing_sl_activation_pct || 0.005,
    trailing_sl_distance_pct: bot.trailing_sl_distance_pct || 0.003,
    neutral_volatility_gate_enabled: bot.neutral_volatility_gate_enabled !== false,
    neutral_volatility_gate_threshold_pct: bot.neutral_volatility_gate_threshold_pct || 5.0,
    recovery_enabled: bot.recovery_enabled !== false,
    entry_gate_enabled: bot.entry_gate_enabled !== false,
    auto_stop: bot.auto_stop || null,
    tp_pct: bot.tp_pct || null,
    // Quick Profit settings
    quick_profit_enabled: bot.quick_profit_enabled || false,
    quick_profit_target: bot.quick_profit_target || null,
    quick_profit_close_pct: bot.quick_profit_close_pct || null,
    quick_profit_cooldown: bot.quick_profit_cooldown || null
  };
  localStorage.setItem(LAST_STARTED_BOT_KEY, JSON.stringify(lastBotData));
  console.log('[LastCoin] Saved to localStorage:', lastBotData);
  updateLastCoinDisplay();
}

// Update the UI display for last coin (with editable inputs)
function updateLastCoinDisplay() {
  const section = $('last-coin-section');
  const symbolEl = $('last-coin-symbol');
  const modeEl = $('last-coin-mode');
  // Get editable input elements
  const gridsEl = $('last-coin-grids');
  const leverageEl = $('last-coin-leverage');
  const investmentEl = $('last-coin-investment');
  const targetEl = $('last-coin-target');
  // New editable fields
  const actEl = $('last-coin-act');
  const distEl = $('last-coin-dist');
  const qpTargetEl = $('last-coin-qp-target');
  const closeEl = $('last-coin-close');
  const cdEl = $('last-coin-cd');
  const atrEl = $('last-coin-atr');

  const stored = localStorage.getItem(LAST_STARTED_BOT_KEY);
  ...
  try {
    const lastBot = JSON.parse(stored);
    if (symbolEl) symbolEl.textContent = lastBot.symbol;
    if (modeEl) {
      modeEl.textContent = lastBot.mode.toUpperCase();
      modeEl.className = 'px-2 py-0.5 text-xs font-medium rounded-full ' +
        (lastBot.mode === 'long' ? 'bg-emerald-600/30 text-emerald-400' :
          lastBot.mode === 'short' ? 'bg-red-600/30 text-red-400' :
            'bg-purple-600/30 text-purple-400');
    }
    // Populate editable input fields with saved values
    if (gridsEl) gridsEl.value = lastBot.grid_count || 10;
    if (leverageEl) leverageEl.value = lastBot.leverage || 3;
    if (investmentEl) investmentEl.value = lastBot.investment || 50;
    if (targetEl) targetEl.value = lastBot.auto_stop_target_usdt || '';
    // Populate new fields (convert from fraction to percentage for display)
    if (actEl) actEl.value = lastBot.trailing_sl_activation_pct ? (lastBot.trailing_sl_activation_pct * 100).toFixed(1) : '';
    if (distEl) distEl.value = lastBot.trailing_sl_distance_pct ? (lastBot.trailing_sl_distance_pct * 100).toFixed(1) : '';
    if (qpTargetEl) qpTargetEl.value = lastBot.quick_profit_target || '';
    if (closeEl) closeEl.value = lastBot.quick_profit_close_pct ? (lastBot.quick_profit_close_pct * 100).toFixed(0) : '';
    if (cdEl) cdEl.value = lastBot.quick_profit_cooldown || '';
    if (atrEl) atrEl.value = lastBot.neutral_volatility_gate_threshold_pct || '';

    if (section) {
      section.classList.remove('hidden');
    }
  } catch (e) {
    console.error('[LastCoin] Failed to parse last bot:', e);
    if (section) section.classList.add('hidden');
  }
}

// Run the last coin again (with editable settings)
async function runLastCoin() {
  const stored = localStorage.getItem(LAST_STARTED_BOT_KEY);
  if (!stored) {
    showToast('No last coin to run', 'error');
    return;
  }

  const btn = $('btn-run-last-coin');
  const textEl = $('run-last-coin-text');

  if (btn) {
    btn.disabled = true;
    btn.classList.add('opacity-75');
  }
  if (textEl) textEl.innerHTML = '<span class="animate-spin inline-block">&#8635;</span> Starting...';

  try {
    const lastBot = JSON.parse(stored);

    // Read modified values from editable inputs
    const gridsEl = $('last-coin-grids');
    const leverageEl = $('last-coin-leverage');
    const investmentEl = $('last-coin-investment');
    const targetEl = $('last-coin-target');
    ...
    // Parse new field values (convert percentage to fraction where needed)
    const actPct = actEl && actEl.value ? parseFloat(actEl.value) / 100 : (lastBot.trailing_sl_activation_pct || 0.005);
    const distPct = distEl && distEl.value ? parseFloat(distEl.value) / 100 : (lastBot.trailing_sl_distance_pct || 0.003);
    const qpTarget = qpTargetEl && qpTargetEl.value ? parseFloat(qpTargetEl.value) : (lastBot.quick_profit_target || null);
    const closePct = closeEl && closeEl.value ? parseFloat(closeEl.value) / 100 : (lastBot.quick_profit_close_pct || null);
    const cdVal = cdEl && cdEl.value ? parseInt(cdEl.value) : (lastBot.quick_profit_cooldown || null);
    const atrPct = atrEl && atrEl.value ? parseFloat(atrEl.value) : (lastBot.neutral_volatility_gate_threshold_pct || 5.0);

    const updatedConfig = {
      id: lastBot.id,
      symbol: lastBot.symbol,
      mode: lastBot.mode,
      lower_price: lastBot.lower_price,
      upper_price: lastBot.upper_price,
      range_mode: lastBot.range_mode || 'trailing',
      profile: lastBot.profile || 'normal',
      // Editable fields (read from inputs)
      grid_count: gridsEl ? parseInt(gridsEl.value) || lastBot.grid_count : lastBot.grid_count,
      leverage: leverageEl ? parseInt(leverageEl.value) || lastBot.leverage : lastBot.leverage,
      investment: investmentEl ? parseFloat(investmentEl.value) || lastBot.investment : lastBot.investment,
      auto_stop_target_usdt: targetEl && targetEl.value ? parseFloat(targetEl.value) : (lastBot.auto_stop_target_usdt || 0),
      // Advanced settings (editable from Last Coin bar)
      auto_direction: lastBot.auto_direction || false,
      auto_pilot: lastBot.auto_pilot || false,
      trailing_sl_enabled: !!lastBot.trailing_sl_enabled,
      trailing_sl_activation_pct: actPct,
      trailing_sl_distance_pct: distPct,
      neutral_volatility_gate_enabled: lastBot.neutral_volatility_gate_enabled !== false,
      neutral_volatility_gate_threshold_pct: atrPct,
      recovery_enabled: lastBot.recovery_enabled !== false,
      entry_gate_enabled: lastBot.entry_gate_enabled !== false,
      auto_stop: lastBot.auto_stop || null,
      tp_pct: lastBot.tp_pct || null,
      // Quick Profit settings (editable from Last Coin bar)
      quick_profit_enabled: lastBot.quick_profit_enabled || false,
      quick_profit_target: qpTarget,
      quick_profit_close_pct: closePct,
      quick_profit_cooldown: cdVal
    };

    // Update bot config first with new settings
    await fetchJSON('/bots', { method: 'POST', body: JSON.stringify(updatedConfig) });

    // Then start the bot
    await fetchJSON('/bots/start', { method: 'POST', body: JSON.stringify({ id: lastBot.id }) });

    // Update localStorage with new values for next time
    lastBot.grid_count = updatedConfig.grid_count;
    lastBot.leverage = updatedConfig.leverage;
    lastBot.investment = updatedConfig.investment;
    lastBot.auto_stop_target_usdt = updatedConfig.auto_stop_target_usdt;
    // Save new editable fields
    lastBot.trailing_sl_activation_pct = updatedConfig.trailing_sl_activation_pct;
    lastBot.trailing_sl_distance_pct = updatedConfig.trailing_sl_distance_pct;
    lastBot.quick_profit_target = updatedConfig.quick_profit_target;
    lastBot.quick_profit_close_pct = updatedConfig.quick_profit_close_pct;
    lastBot.quick_profit_cooldown = updatedConfig.quick_profit_cooldown;
    lastBot.neutral_volatility_gate_threshold_pct = updatedConfig.neutral_volatility_gate_threshold_pct;
    localStorage.setItem(LAST_STARTED_BOT_KEY, JSON.stringify(lastBot));

    await refreshBots();
    showToast(`Started ${lastBot.symbol} (${lastBot.mode.toUpperCase()}) - Grids:${updatedConfig.grid_count} Lev:${updatedConfig.leverage}x`, 'success');
  } catch (error) {
    ...
    if (textEl) textEl.innerHTML = '🚀 Run Again';
  }
}
...
function configureLivePolling(useLiveFeed) {
  ...
  const quickMs = useLiveFeed ? LIVE_CONNECTED_PNL_REFRESH_MS : LIVE_FALLBACK_PNL_REFRESH_MS;
  const fullMs = useLiveFeed ? LIVE_CONNECTED_FULL_REFRESH_MS : LIVE_FALLBACK_FULL_REFRESH_MS;

  liveQuickRefreshInterval = setInterval(refreshPnlQuick, quickMs);
  liveFullRefreshInterval = setInterval(refreshAll, fullMs);
}
...
function connectLiveFeed() {
  ...
  ["ticker", "position", "execution"].forEach((eventName) => {
    source.addEventListener(eventName, () => {
      setLiveFeedConnected(true);
      updateLastRefreshTime();
      scheduleLiveQuickRefresh(150);
    });
  });

  ["order", "health"].forEach((eventName) => {
    source.addEventListener(eventName, () => {
      setLiveFeedConnected(true);
      updateLastRefreshTime();
      scheduleLiveFullRefresh(150);
    });
  });
}
...
function updateRunningBotsStatus(bots) {
  ...
  container.innerHTML = runningBots.map(bot => {
    ...
    // Timer - show accumulated runtime (uses backend-computed runtime_hours which properly pauses when bot stops)
    const elapsed = formatRuntimeHours(bot.runtime_hours);
    ...
    return `<div class="flex flex-col gap-0.5 p-2 rounded bg-slate-800 border border-slate-700 min-w-[120px]">
      ...
      <div class="flex items-center gap-1">
        ...
        ${elapsed && elapsed !== '-' ? `<span class="text-[9px] text-cyan-400">${elapsed}</span>` : ''}
      </div>
...
function fillBotForm(bot) {
  ...
  if (trailingSLCheckbox) {
    trailingSLCheckbox.checked = !!bot.trailing_sl_enabled;
  }
  ...
  // Quick Profit settings
  const qpEnabled = $("bot-quick-profit-enabled");
  if (qpEnabled) {
    qpEnabled.checked = bot.quick_profit_enabled || false;
  }
  const qpTarget = $("bot-quick-profit-target");
  if (qpTarget) {
    qpTarget.value = bot.quick_profit_target != null ? bot.quick_profit_target.toFixed(2) : "";
  }
  const qpClosePct = $("bot-quick-profit-close-pct");
  if (qpClosePct) {
    qpClosePct.value = bot.quick_profit_close_pct != null ? (bot.quick_profit_close_pct * 100).toFixed(0) : "";
  }
  const qpCooldown = $("bot-quick-profit-cooldown");
  if (qpCooldown) {
    qpCooldown.value = bot.quick_profit_cooldown != null ? bot.quick_profit_cooldown : "";
  }

  const recoveryToggle = $("bot-recovery-enabled");
  if (recoveryToggle) {
    recoveryToggle.checked = !!bot.recovery_enabled;
  }
  ...
  if (entryGateEnabled) {
    entryGateEnabled.checked = !!bot.entry_gate_enabled;
  }
...
function resetBotForm() {
  ...
  // Quick Profit - reset to disabled by default
  const qpEnabled = $("bot-quick-profit-enabled");
  if (qpEnabled) qpEnabled.checked = false;
  const qpTarget = $("bot-quick-profit-target");
  if (qpTarget) qpTarget.value = "";
  const qpClosePct = $("bot-quick-profit-close-pct");
  if (qpClosePct) qpClosePct.value = "";
  const qpCooldown = $("bot-quick-profit-cooldown");
  if (qpCooldown) qpCooldown.value = "";
...
async function saveBot() {
  ...
  const botData = {
    ...
    // Quick Profit settings
    quick_profit_enabled: $("bot-quick-profit-enabled") ? $("bot-quick-profit-enabled").checked : false,
    quick_profit_target: $("bot-quick-profit-target") && $("bot-quick-profit-target").value
      ? parseFloat($("bot-quick-profit-target").value)
      : null,
    quick_profit_close_pct: $("bot-quick-profit-close-pct") && $("bot-quick-profit-close-pct").value
      ? parseFloat($("bot-quick-profit-close-pct").value) / 100
      : null,
    quick_profit_cooldown: $("bot-quick-profit-cooldown") && $("bot-quick-profit-cooldown").value
      ? parseInt($("bot-quick-profit-cooldown").value)
      : null,
  };
  ...
  try {
    ...
    await fetchJSON("/bots", { method: "POST", body: JSON.stringify(botData) });
    resetBotForm();
    await refreshBots();
    showToast(isEdit ? `✅ ${savedSymbol} bot updated!` : `✅ ${savedSymbol} bot created!`, "success");

    // Scroll to the bot row and highlight it
    setTimeout(() => {
      const rowId = `bot-row-${savedSymbol}`;
      const row = document.getElementById(rowId);
      if (row) {
        row.scrollIntoView({ behavior: "smooth", block: "center" });
        row.style.transition = "all 0.4s ease";
        row.style.backgroundColor = "rgba(16, 185, 129, 0.2)";
        row.style.boxShadow = "0 0 15px rgba(16, 185, 129, 0.5)";
        ...
        console.warn(`Could not find row by ID: ${rowId}`);
        // Fallback to text search if ID fails
        const rows = document.querySelectorAll("#bots-body tr");
        for (const r of rows) {
          if (r.textContent.includes(savedSymbol)) {
            r.scrollIntoView({ behavior: "smooth", block: "center" });
            r.classList.add("bg-emerald-500/20");
            setTimeout(() => r.classList.remove("bg-emerald-500/20"), 4000);
            break;
          }
        }
      }
...
async function botAction(action, botId, event, silent = false) {
  ...
  // Capture bot data BEFORE the action (for quick re-run feature)
  console.log('[LastCoin] botAction called:', action, botId);
  console.log('[LastCoin] window._lastBots:', window._lastBots);
  const botToTrack = (action === "start") ? window._lastBots?.find(b => b.id === botId) : null;
  console.log('[LastCoin] botToTrack:', botToTrack);

  try {
    await fetchJSON(`/bots/${action}`, { method: "POST", body: JSON.stringify({ id: botId }) });
    await refreshBots();

    // Save last started bot for quick re-run feature
    if (botToTrack) {
      console.log('[LastCoin] Saving bot to track:', botToTrack);
      saveLastStartedBot(botToTrack);
    } else {
      console.log('[LastCoin] No bot to track found');
    }
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    // Close All PnL modal first (check this one first since it may be on top)
...
async function loadAnalytics(period) {
  ...
  ['7d', '30d', 'all'].forEach(p => {
    ...
    if (btn) {
      if (p === period) {
        btn.className = "px-3 py-1 text-xs font-medium rounded-md bg-emerald-600 text-white shadow-lg";
      } else {
        btn.className = "px-3 py-1 text-xs font-medium rounded-md transition-all text-slate-400 hover:text-slate-200";
      }
    }
  });

  try {
    const data = await fetchJSON(`/pnl/analytics?period=${period}`);
    if (!data) return;

    // Update aggregate stats
    const netPnl = formatPnL(data.metrics.net_pnl);
    $("ana-net-pnl").textContent = netPnl.text;
    $("ana-net-pnl").className = `text-2xl font-bold ${netPnl.class}`;

    $("ana-win-rate").textContent = `${data.metrics.win_rate}%`;
    $("ana-profit-factor").textContent = data.metrics.profit_factor;

    const mdd = formatNumber(data.metrics.max_drawdown);
    $("ana-mdd").textContent = `$${mdd}`;
...
async function loadBotLog() {
  ...
  try {
    const res = await fetch(`/api/bot/log?lines=${numLines}`);
...
window.addEventListener("load", () => {
  ...
  updateLastCoinDisplay();  // Show last coin quick-run bar if available
  refreshAll();
  startLiveTimer();
  configureLivePolling(false);
  connectLiveFeed();
...
async function refreshBotDetailLogs() {
  ...
  try {
    const res = await fetch(`/api/bots/${window.currentDetailBotId}/logs?limit=100`);
    const data = await res.json();
